...
`DEBUG=1` переопределяет все `timeout_seconds` из сценария на 30 секунд и ускоряет фоновую проверку отложенных сообщений. В обычном режиме таймауты берутся из `data/flow.yaml`.
...
- таймауты игнорирования: в обычном режиме через 24 часа молчания запускается штатный 30-секундный таймер `КК-1 → КК-17 → КК-6`, `КК-6 → КК-18 → КК-20`, `КК-20 → КК-27 → КК-28`; при `DEBUG=1` эти таймауты становятся 30-секундными.

...
`DEBUG=1` переопределяет все `timeout_seconds` и `delay_seconds` из сценария на 30 секунд и ускоряет фоновую проверку отложенных сообщений. В обычном режиме тайминги берутся из `data/flow.yaml`.
...
- таймауты игнорирования и timer-узлы: в обычном режиме все отложенные переходы занимают 24 часа; при `DEBUG=1` эти тайминги становятся 30-секундными.
